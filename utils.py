...
@st.cache_resource(ttl=time_to_live)
def load_round_data(program_options, csv_path='all_rounds.csv'):
    round_data = pd.read_csv(csv_path)
    round_data = round_data[round_data['program'].isin(program_options)]
    ...
    starting_time = pd.to_datetime(round_data['starting_time'].max())
    ...
    dfv['seconds_since_start'] = 0
    for _, row in round_data.iterrows():
        dfv.loc[dfv['round_id'] == row['round_id'], 'seconds_since_start'] = (dfv['block_timestamp'] - pd.to_datetime(row['starting_time'])).dt.total_seconds()
        dfv.loc[dfv['seconds_since_start'] < 0, 'seconds_since_start'] = 0
    dfv['hours_since_start'] = dfv['seconds_since_start'] // 3600
